...
COMPONENT = 1; %フィットモデル式の拡散成分の数（１か２を選択。詳細は"script>fitting>fitting_temporal.mlx"を参照。）
choice = 3; %計算したいACFの種類を選択
% ①時間相関(デフォルト。各ピクセルの時間相関の平均。) ②時間相関(任意の１ピクセル）③時空間相関

%②：任意の1ピクセルの時間相関を選択した場合は　要変更
if choice == 2
    constant_X = 3 ; %任意の1ピクセルconstant_Xを選択( 1 < constant_X < PIXEL数)。このピクセルだけの時間相関を計算する
end
%% パスを通す
...
for idx=1:length(filename_array)
    ...
    if choice == 1 %%各ピクセルにおける時間相関の平均を計算する
      ...
      elseif choice == 2 %定点X（＝1ピクセル)を選択。このピクセルだけの時間相関を計算する
        ...
        [TAU, COR] = constantX_temporal_correlation(XT_oneline, TIME_SCALE, NUMBER_TAU); %XTの次元:TIMESERIES×1
        %Run Fitting
         run("fitting_temporal.mlx")
        %Run Plot
         run("temporal_plots.mlx")
